...
# Pregunta 14
fqTable <- db |> 
  dplyr::select(p13, p14) |> 
  dplyr::group_by(p13, p14) |>
...
fqTable$Categoria <- factor(fqTable$Categoria, 
                            levels = c("Nada atractivo",
                                       "Poco atractivo",
                                       "Neutro",
                                       "Atractivo",
                                       "Muy atractivo"), ordered = T)
lvl <- c("Nada atractivo",
         "Poco atractivo",
         "Neutro",
         "Atractivo",
         "Muy atractivo")
...
# Pregunta 15
fqTable <- db |> 
  dplyr::select(p13, p15) |> 
  dplyr::group_by(p13, p15) |>
  dplyr::summarise(frecuencia = n(), .groups = "drop") |>
  plyr::mutate(porcentaje = (frecuencia / sum(frecuencia)) * 100); fqTable
names(fqTable) <- c("Variable", "Categoria", "Frecuencia", "Porcentaje")
fqTable$Categoria <- as.factor(fqTable$Categoria)
...
fqTable$Categoria <- factor(fqTable$Categoria, 
                            levels = c("Nada agradable",
                                       "Poco agradable",
                                       "Neutro",
                                       "Agradable",
                                       "Muy agradable"), ordered = T)
lvl <- c("Nada agradable",
         "Poco agradable",
         "Neutro",
         "Agradable",
         "Muy agradable")
